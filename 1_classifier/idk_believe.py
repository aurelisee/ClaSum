...
def EvaluationBehaviorCap(dataList, domainwordThreshold):
    sheetNum = len(dataList)

    for i in range(sheetNum):
        #print("Processing :", sheetName[i])
        SenList, lemSenList, senNumberList, authorList, senInvolvecd, senVecList, senOPscore, tfidfWordList, tfidfScoreList, ignoreList = BugSumSigDataInput(dataList, i)
        ...
        dataList[i]["Believability"] = FscoreList
    return dataList
...
def DescriptionCite(sentenceInvolved, sentencelist, domainwordlist, evaluationTimeList, evaluationList, EvaluatorSentenceNumberList, CombinedQuotedSentenceList):
    ...
    for DescSentenceNumber in sentenceInvolved[0]:
        ...
        for Dword in domainwordlist:
            ...
            if Dword in str(sentencelist[DescSentenceNumber]):
        ...
        for commentnumber in range(1,commentnum):
            ...
            for i in range(senNumInComment):
                ...
                for TargetDomainWord in imptDomainWordList:
                    if (TargetDomainWord in str(sentencelist[sentencenumber])):#This "sentencenumber" is also have certain value as the evaluator sentence
...
def SenDomainWordCheck(domainwordList, sena, senb):
    ...
    for i in domainwordList:
        if i in str(sena) and i in str(senb):
            #print("Domainword:", i)
            return i
    return "False!!!!"
...
def EvaluationBehaviorCapture(datasetName, domainwordThreshold):
    sheetNameList, dataList = DataInput(datasetName)
    # 원래 코드에는 0.1로 되어있었음 (논문에는 0.08)
    dataList = EvaluationBehaviorCap(dataList, 0.08)
